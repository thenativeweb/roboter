...
const createTest = function ({ task, testCase, directory, roboterPackagePath }) {
  ...
  test(`${testCase.replace(/-/ug, ' ')}.`, async () => {
    ...
    await runCommand('npm install --no-package-lock --silent', { cwd: testDirectory });
    await runCommand(`npm install ${roboterPackagePath} --no-package-lock`, { cwd: testDirectory });
