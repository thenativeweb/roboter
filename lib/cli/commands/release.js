...
const release = {
  ...
  async run (options) {
    ...
    try {
      ...
      await git.checkIfOnMaster({ directory, ui });
      ...
      await git.tag({ directory, name: versions.new, message: `Create tag for version ${versions.new}.`, ui });
