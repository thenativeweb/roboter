...
const getLicense = async function (directory) {
  ...
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    const fileStat = await lstat(path.join(directory, file));

    if (!fileStat.isFile()) {
      continue;
    }

    if (/^(copying|license|readme)(\.markdown|\.md|\.txt)?$/i.test(file)) {
      const licenseFile = await readFile(path.join(directory, file), { encoding: 'utf8' }),
...
const license = {
  ...
  async run (options) {
    ...
    try {
      ...
      const entries = await readdir(path.join(directory, 'node_modules'));
      const incompatiblePackages = [];

      for (let i = 0; i < entries.length; i++) {
        const pkg = entries[i],
              pkgPath = path.join(directory, 'node_modules', pkg);
        ...
        const isDivergent = myLicense !== pkgLicense;
        const licenseFromWhiteList = getLicenseFromWhiteList({ pkg, pkgVersion });

        if (
          isDivergent &&
          compatibleLicences.includes(pkgLicense) || compatibleLicences.includes(licenseFromWhiteList)
        ) {
          continue;
        } else {
          incompatiblePackages.push({ pkg, pkgVersion, pkgLicense });
        }
      }
      ...
      if (incompatiblePackages.length > 0) {
        ...
        table.push([ 'Package', 'Version', 'License' ]);
        table.push([]);

        incompatiblePackages.forEach(({ pkg, pkgVersion, pkgLicense }) => {
          table.push([ pkg, pkgVersion, pkgLicense ]);
        });
