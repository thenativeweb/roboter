  ...
  async run (options) {
    ...
    if (watch) {
      await watchFilesAndExecute({
        ...
        files: await testSteps.testCode.getFiles({ directory }),
        execute: async () => await testSteps.testCode({ directory, type, printSuccess: true, ui }),
        executeOnStart: true
      });
    }

    try {
      await testSteps.testCode({ directory, type, ui });
