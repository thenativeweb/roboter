  ...
  await new Promise((resolve, reject) => {
    depcheck(directory, depcheckOptions, ({
      ...
      if (invalidDirs.length > 0) {
        hasErrorOccured = true;
        ui.info(`Unable to access some dirs: ${invalidDirs.join(', ')}`);
      }
      ...
      if (dependencies.length > 0 || devDependencies.length > 0) {
        ...
        ui.info(`Following dependencies are unused or missing:\n\n${allUnusedDeps.join(',\n')}`);
      }

      if (hasErrorOccured) {
        ui.warn('You have unused or missing dependencies.');
