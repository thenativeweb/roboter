...
const coverage = function (roboter, userConfiguration) {
  ...
  const runCoveragePre = function () {
    ...
    return gulp.src(configuration.src).
      pipe(istanbul({ includeUntested: configuration.includeUntested })).
  ...
  const runCoverage = function (options, done) {
    ...
    gulp.src(configuration.test).
      ...
      pipe(istanbul.enforceThresholds({ thresholds: configuration.thresholds })).
      on('end', done);
  };
