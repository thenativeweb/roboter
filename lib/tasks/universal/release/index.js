...
const release = function (roboter, userConfiguration) {
  ...
  gulp.task('_release-precompile', done => {
    ...
    shell.exec(`node_modules/.bin/babel "${configuration.srcDir}" --out-dir "${configuration.distributionDir}" --copy-files --presets=es2015,es2016,es2017,react`, (exitCode, stdout, stderr) => {
      if (exitCode !== 0) {
