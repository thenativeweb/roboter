...
const errors = require('../../errors'),
      files = require('../../files'),
      filesToWatch = require('../../sourceFiles'),
  ...
  try {
    await runCommand(oneLineInlineLists`
        npx mocha
          ...
          ${await asyncAF(testFiles).filterAF(async testFile => (await files.get({ directory, pattern: testFile })).length > 0)}
      `, {
      cwd: directory,
